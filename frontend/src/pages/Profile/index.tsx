  ...
  return (
    <div className="mx-auto w-full max-w-[448px] rounded-2xl border border-[#D8E1EE] bg-white p-8">
      <div className="mb-8 flex flex-col items-center gap-3 border-b border-[#E2E8F0] pb-8">
        ...
        <h1 className="text-[18px] font-bold leading-[1.05] text-slate-900">
          {user?.fullname || "Seu perfil"}
        </h1>
        <p className="text-[16px] text-slate-500">{user?.email}</p>
      </div>
      ...
      <Button
        variant="outline"
        className="mt-4 h-12 w-full gap-2 bg-white hover:bg-red-50"
        onClick={leave}
      >
        <LogOut className="h-4 w-4 text-red-500" /> Sair da conta
      </Button>
    </div>
  );
}
